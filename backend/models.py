...
class Student(models.Model):
    ...
    phone_validate = RegexValidator(
        regex=r"^\d{10}$",
        message="Enter your 10 digit phone number without country codes and symbols",
    )

    phone_number = models.CharField(
        blank=False, max_length=10, validators=[phone_validate,]
    )
    ...
    year_of_study = models.CharField(
        blank=False, max_length=1, choices=YEAR_OF_STUDY_CHOICES
    )
...
class Company(models.Model):
    ...
    poc = models.CharField(
        blank=False, max_length=255, default="", verbose_name="person of contact"
    )
...
class Job(models.Model):
    ...
    is_flexi = models.BooleanField(
        verbose_name="flexible start and end date", null=False, default=False
    )
    ...
    num_pos = models.PositiveSmallIntegerField(
        verbose_name="number of open positions", default=1, null=False
    )
